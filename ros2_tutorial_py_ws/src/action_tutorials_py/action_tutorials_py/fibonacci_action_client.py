...
class FibonacciActionClient(Node):
    ...
    def __init__(self):
        ...
        # validate params before they're set and react after they're set
        ...
        # parameter, can change at runtime if desired
    ...
    def send_goal(self, order):
        ...
        # blocks till action server is ready for request
        self._action_client.wait_for_server()
        # send goal to server async (does not block client program while waiting)
        # returns a future object
        self._send_goal_future = self._action_client.send_goal_async(
            goal_msg,
            # every time the server send feeback, this function is called asynchronously
            feedback_callback=self.feedback_callback)
        # attaches a callback to the future 
        # when goal is accepted or rejected by the server, the goal_response_callback functin runs
    ...
    def goal_response_callback(self, future):
        ...
        # I know goal is running, tell me later when it's done
        self._get_result_future = goal_handle.get_result_async()

        # when action finishes, run get result callback
