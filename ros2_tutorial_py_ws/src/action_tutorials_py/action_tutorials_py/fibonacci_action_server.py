...
class FibonacciActionServer(Node):
    ...
    def __init__(self):
        ...
        self._action_server = ActionServer(
            ...
            Fibonacci,
            'fibonacci',
            # blocks it's own thread, not cancel_callback worker thread
            self.execute_callback, # runs when a goal is accepted
            # runs in a worker thread when triggered, not main thread
            cancel_callback=self.cancel_callback) # runs when client requests cancellation
        # validate param types and values
